...
#ifndef HERO_LOCALIZATION_LOCALIZATION_CONFIG_H
#define HERO_LOCALIZATION_LOCALIZATION_CONFIG_H
...
struct LocalizationConfig {
  void GetParam(ros::NodeHandle *nh) {
      ...
      std::string initial_pose_x_str = "initial_pose_x";
      std::string initial_pose_y_str = "initial_pose_y";
      std::string initial_pose_a_str = "initial_pose_a";

    if(nh->getNamespace().c_str()!="/"&&nh->getNamespace().size()>5)
      {
        initial_pose_x_str = initial_pose_x_str + '_' + nh->getNamespace().substr(2);
        initial_pose_y_str = initial_pose_y_str + '_' + nh->getNamespace().substr(2);
        initial_pose_a_str = initial_pose_a_str + '_' + nh->getNamespace().substr(2);
        ROS_INFO("initial_pose rename!,ns = %s",nh->getNamespace().c_str());
      }
      else
      {
      ...
      nh->param<double>(initial_pose_y_str, initial_pose_y, 1);
      nh->param<double>(initial_pose_a_str, initial_pose_a, 0);
...
#endif //HERO_LOCALIZATION_LOCALIZATION_CONFIG_H
