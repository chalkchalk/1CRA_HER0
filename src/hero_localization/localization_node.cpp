...
bool LocalizationNode::Init() {
  ...
  //nh_->param<bool>("use_map_topic", use_map_topic, true);
  ...
  {
      ROS_INFO("ns:%s",nh_.getNamespace().c_str());
      base_frame_ = nh_.getNamespace().substr(1) + '/' + base_frame_;
      //global_frame_ = nh_.getNamespace().substr(1) + '/' + global_frame_;
      odom_frame_ = nh_.getNamespace().substr(1) + '/' + odom_frame_;
      ROS_INFO("base_frame_:%s",base_frame_.c_str());
      ROS_INFO("global_frame_:%s",global_frame_.c_str());
      ROS_INFO("odom_frame_:%s",odom_frame_.c_str());
  }
...
int main(int argc, char **argv) {
  hero_localization::GLogWrapper glog_wrapper(argv[0]);
  ros::init(argc, argv, "localization_node");
  hero_localization::LocalizationNode localization_node("localization_node");
