...
namespace hero_base{
Gimbal::Gimbal(std::shared_ptr<hero_sdk::Handle> handle):
    handle_(handle){
  SDK_Init();
  ROS_Init();
}
...
void Gimbal::SDK_Init(){
  ...
  verison_client_ = handle_->CreateClient<hero_sdk::cmd_version_id,hero_sdk::cmd_version_id>
      (UNIVERSAL_CMD_SET, CMD_REPORT_VERSION,
       MANIFOLD2_ADDRESS, GIMBAL_ADDRESS);
  hero_sdk::cmd_version_id version_cmd;
  version_cmd.version_id=0;
  auto version = std::make_shared<hero_sdk::cmd_version_id>(version_cmd);
  verison_client_->AsyncSendRequest(version,
                                    [](hero_sdk::Client<hero_sdk::cmd_version_id,
                                                           hero_sdk::cmd_version_id>::SharedFuture future) {
  ...
  handle_->CreateSubscriber<hero_sdk::cmd_gimbal_info>(GIMBAL_CMD_SET, CMD_PUSH_GIMBAL_INFO,
                                                          GIMBAL_ADDRESS, BROADCAST_ADDRESS,
                                                          std::bind(&Gimbal::GimbalInfoCallback, this, std::placeholders::_1));

  gimbal_angle_pub_ = handle_->CreatePublisher<hero_sdk::cmd_gimbal_angle>(GIMBAL_CMD_SET, CMD_SET_GIMBAL_ANGLE,
                                                                              MANIFOLD2_ADDRESS, GIMBAL_ADDRESS);
  gimbal_mode_pub_ = handle_->CreatePublisher<hero_sdk::gimbal_mode_e>(GIMBAL_CMD_SET, CMD_SET_GIMBAL_MODE,
                                                                          MANIFOLD2_ADDRESS, GIMBAL_ADDRESS);
  fric_wheel_pub_ = handle_->CreatePublisher<hero_sdk::cmd_fric_wheel_speed>(GIMBAL_CMD_SET, CMD_SET_FRIC_WHEEL_SPEED,
                                                                                MANIFOLD2_ADDRESS, GIMBAL_ADDRESS);
  gimbal_shoot_pub_ = handle_->CreatePublisher<hero_sdk::cmd_shoot_info>(GIMBAL_CMD_SET, CMD_SET_SHOOT_INFO,
                                                                            MANIFOLD2_ADDRESS, GIMBAL_ADDRESS);

  heartbeat_pub_ = handle_->CreatePublisher<hero_sdk::cmd_heartbeat>(UNIVERSAL_CMD_SET, CMD_HEARTBEAT,
                                                                        MANIFOLD2_ADDRESS, GIMBAL_ADDRESS);
  heartbeat_thread_ = std::thread([this]{
                                    hero_sdk::cmd_heartbeat heartbeat;
...
void Gimbal::GimbalInfoCallback(const std::shared_ptr<hero_sdk::cmd_gimbal_info> gimbal_info){
...
void Gimbal::GimbalAngleCtrlCallback(const hero_msgs::GimbalAngle::ConstPtr &msg){

  hero_sdk::cmd_gimbal_angle gimbal_angle;
...
bool Gimbal::SetGimbalModeService(hero_msgs::GimbalMode::Request &req,
                                  hero_msgs::GimbalMode::Response &res){
  hero_sdk::gimbal_mode_e gimbal_mode = static_cast<hero_sdk::gimbal_mode_e>(req.gimbal_mode);
  gimbal_mode_pub_->Publish(gimbal_mode);
  res.received = true;
  return true;
}
bool Gimbal::CtrlFricWheelService(hero_msgs::FricWhl::Request &req,
                                  hero_msgs::FricWhl::Response &res){
  hero_sdk::cmd_fric_wheel_speed fric_speed;
  if(req.open){
...
bool Gimbal::CtrlShootService(hero_msgs::ShootCmd::Request &req,
                              hero_msgs::ShootCmd::Response &res){
  hero_sdk::cmd_shoot_info gimbal_shoot;
  uint16_t default_freq = 1500;
  switch(static_cast<hero_sdk::shoot_cmd_e>(req.mode)){
    case hero_sdk::SHOOT_STOP:
      gimbal_shoot.shoot_cmd = hero_sdk::SHOOT_STOP;
      gimbal_shoot.shoot_add_num = 0;
      gimbal_shoot.shoot_freq = 0;
      break;
    case hero_sdk::SHOOT_ONCE:
      if(req.number!=0){
        gimbal_shoot.shoot_cmd = hero_sdk::SHOOT_ONCE;
      ...
      break;
    case hero_sdk::SHOOT_CONTINUOUS:
      gimbal_shoot.shoot_cmd = hero_sdk::SHOOT_CONTINUOUS;
      gimbal_shoot.shoot_add_num = req.number;
      gimbal_shoot.shoot_freq = default_freq;
      break;
    default:
