...
namespace hero_base{
Chassis::Chassis(std::shared_ptr<hero_sdk::Handle> handle):
    handle_(handle){
  SDK_Init();
  ROS_Init();
}
...
void Chassis::SDK_Init(){
  ...
  handle_->CreateSubscriber<hero_sdk::cmd_chassis_info>(CHASSIS_CMD_SET, CMD_PUSH_CHASSIS_INFO,
                                                           CHASSIS_ADDRESS, MANIFOLD2_ADDRESS,
                                                           std::bind(&Chassis::ChassisInfoCallback, this, std::placeholders::_1));
  handle_->CreateSubscriber<hero_sdk::cmd_uwb_info>(COMPATIBLE_CMD_SET, CMD_PUSH_UWB_INFO,
                                                       CHASSIS_ADDRESS, MANIFOLD2_ADDRESS,
                                                       std::bind(&Chassis::UWBInfoCallback, this, std::placeholders::_1));

  chassis_speed_pub_ = handle_->CreatePublisher<hero_sdk::cmd_chassis_speed>(CHASSIS_CMD_SET, CMD_SET_CHASSIS_SPEED,
                                                                                MANIFOLD2_ADDRESS, CHASSIS_ADDRESS);
  chassis_spd_acc_pub_ = handle_->CreatePublisher<hero_sdk::cmd_chassis_spd_acc>(CHASSIS_CMD_SET, CMD_SET_CHASSIS_SPD_ACC,
                                                                                    MANIFOLD2_ADDRESS, CHASSIS_ADDRESS);
...
void Chassis::ChassisInfoCallback(const std::shared_ptr<hero_sdk::cmd_chassis_info> chassis_info){
...
void Chassis::UWBInfoCallback(const std::shared_ptr<hero_sdk::cmd_uwb_info> uwb_info){
...
void Chassis::ChassisSpeedCtrlCallback(const geometry_msgs::Twist::ConstPtr &vel){
  hero_sdk::cmd_chassis_speed chassis_speed;
...
void Chassis::ChassisSpeedAccCtrlCallback(const hero_msgs::TwistAccel::ConstPtr &vel_acc){
  hero_sdk::cmd_chassis_spd_acc chassis_spd_acc;
