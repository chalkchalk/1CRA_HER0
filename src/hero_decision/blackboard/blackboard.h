...
class Blackboard {
 public:
  ...
  typedef hero_costmap::CostmapInterface CostMap;
  typedef hero_costmap::Costmap2D CostMap2D;
  explicit Blackboard(const std::string &proto_file_path):
    ...
    hero_common::ReadProtoFromTextFile(proto_file_path, &decision_config);
    ...
    if (!decision_config.simulate()){
      ...
      armor_detection_actionlib_client_.sendGoal(armor_detection_goal_,
                                                 actionlib::SimpleActionClient<hero_msgs::ArmorDetectionAction>::SimpleDoneCallback(),
                                                 actionlib::SimpleActionClient<hero_msgs::ArmorDetectionAction>::SimpleActiveCallback(),
  ...
  // Enemy
  void ArmorDetectionFeedbackCallback(const hero_msgs::ArmorDetectionFeedbackConstPtr& feedback){
 ...
 private:
  ...
  //! Enemy info
  actionlib::SimpleActionClient<hero_msgs::ArmorDetectionAction> armor_detection_actionlib_client_;
  hero_msgs::ArmorDetectionGoal armor_detection_goal_;
...
#endif //HERO_DECISION_BLACKBOARD_H
