...
GimbalExecutor::GimbalExecutor():excution_mode_(ExcutionMode::IDLE_MODE),
  ...
  ros::NodeHandle nh;
  cmd_gimbal_angle_pub_ = nh.advertise<hero_msgs::GimbalAngle>("cmd_gimbal_angle", 1);
  cmd_gimbal_rate_pub_  = nh.advertise<hero_msgs::GimbalRate>("cmd_gimbal_rate", 1);

}

void GimbalExecutor::Execute(const hero_msgs::GimbalAngle &gimbal_angle){
  excution_mode_ = ExcutionMode::ANGLE_MODE;
  cmd_gimbal_angle_pub_.publish(gimbal_angle);
}

void GimbalExecutor::Execute(const hero_msgs::GimbalRate &gimbal_rate){
