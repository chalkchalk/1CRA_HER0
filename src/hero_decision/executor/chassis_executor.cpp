...
{
  ros::NodeHandle nh;
  cmd_vel_acc_pub_ = nh.advertise<hero_msgs::TwistAccel>("cmd_vel_acc", 100);
...
void ChassisExecutor::Execute(const hero_msgs::TwistAccel &twist_accel){
  if( execution_mode_ == ExcutionMode::GOAL_MODE){
    Cancel();
  }
...
void ChassisExecutor::GlobalPlannerFeedbackCallback(const hero_msgs::GlobalPlannerFeedbackConstPtr& global_planner_feedback){
