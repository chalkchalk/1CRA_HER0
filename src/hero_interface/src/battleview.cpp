...
void BattleView::LeftButtonRelease(int x, int y)
{
   ...
   qNode_->isShooting = false;
    ...
    {
      std::vector<std::string> nearby_robots = FindRobotInViewRange(x,y,40);
       if(nearby_robots.empty())
       {
         SetRobotGoalPoint(x,y);
       }
       else
       {
         SetRobotAttackTarget(nearby_robots[0]);
       }
    }
    //printf("release\n");
}

std::vector<std::string> BattleView::FindRobotInViewRange(int x, int y, double max_distance)
{
  std::vector<std::string> robot_names;
  for (auto it = robots_.begin(); it != robots_.end(); ++it)
  {
   if(hero_common::PointDistance(x,y,
                                 PoseToMapPoint((*it)->pose,background.size()).x(),
                                  PoseToMapPoint((*it)->pose,background.size()).y())<max_distance)
   {
      robot_names.emplace_back((*it)->name);
   }
  }
  return robot_names;
}
...
void BattleView::SetRobotAttackTarget(std::string target)
{
  for (auto it = robots_.begin(); it != robots_.end(); ++it) {
      if((*it)->selected)
      {
        (*it)->target = target;
        qNode_->AttackRobot((*it)->index,target);
        (*it)->attacking_ = true;
      }
  }
}
