...
ErrorInfo GlobalPlannerNode::Init() {
  ...
  std::string full_path = ros::package::getPath("hero_planning") + "/global_planner/config/global_planner_config.prototxt";
  ...
  ROS_INFO("path_.header.frame_id :%s",path_.header.frame_id.c_str());
  return ErrorInfo(ErrorCode::OK);
}

void GlobalPlannerNode::GoalCallback(const hero_msgs::GlobalPlannerGoal::ConstPtr &msg) {
  ...
  while (ros::ok()) {
    ...
    if(node_state == NodeState::RUNNING || node_state == NodeState::SUCCESS || node_state == NodeState::FAILURE) {
      hero_msgs::GlobalPlannerFeedback feedback;
      hero_msgs::GlobalPlannerResult result;
...
void GlobalPlannerNode::PlanThread() {
  ...
  while (ros::ok()) {
    ...
    {
      std::unique_lock<hero_costmap::Costmap2D::mutex_t> lock(*(costmap_ptr_->GetCostMap()->GetMutex()));
