...
class GlobalPlannerTest{
 public:
  ...
  void DoneCallback(const actionlib::SimpleClientGoalState& state,  const hero_msgs::GlobalPlannerResultConstPtr& result){
    ROS_INFO("The goal is done with %s!",state.toString().c_str());
  }
  void ActiveCallback() {
    ROS_INFO("Action server has recived the goal, the goal is active!");
  }
  void FeedbackCallback(const hero_msgs::GlobalPlannerFeedbackConstPtr& feedback){
 ...
 private:
  ...
  hero_msgs::GlobalPlannerGoal command_;
  actionlib::SimpleActionClient<hero_msgs::GlobalPlannerAction> global_planner_actionlib_client_;
};
