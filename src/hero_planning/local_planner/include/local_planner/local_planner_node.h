...
#ifndef HERO_PLANNING_LOCAL_PLANNER_NODE_H
#define HERO_PLANNING_LOCAL_PLANNER_NODE_H
...
class LocalPlannerNode {
 public:
   ...
   */
  void ExcuteCB(const hero_msgs::LocalPlannerGoal::ConstPtr &command);

  /**
   * @brief local planner algorithm's error callback function
   * @param algorithm_error_info local planner algorithm's error info
   */
  void AlgorithmCB(const hero_common::ErrorInfo &algorithm_error_info);
   ...
   */
  void SetNodeState(const hero_common::NodeState& node_state);

  /**
   * @brief Get local planner node state
   * @return State of local planner node
   */
  hero_common::NodeState GetNodeState();

  /**
   * @brief Set Error info if error occur
   * @param error_info error info
   */
  void SetErrorInfo(const hero_common::ErrorInfo error_info);

  /**
   * Get error info
   * @return Error in local planner node
   */
  hero_common::ErrorInfo GetErrorInfo();
 ...
 private:
  ...
  actionlib::SimpleActionServer<hero_msgs::LocalPlannerAction> as_;
  ...
  std::mutex plan_mtx_;
  //! node state
  hero_common::NodeState node_state_;
  //! error info
  hero_common::ErrorInfo node_error_info_;
  //! local cost map
  std::shared_ptr<hero_costmap::CostmapInterface> local_cost_;
  //! tf pointer
...
#endif //HERO_PLANNING_LOCAL_PLANNER_NODE_H
