...
LocalPlannerNode::LocalPlannerNode() :
 //   local_planner_nh_("~"),
    ...
    initialized_(false), node_state_(hero_common::NodeState::IDLE),
    node_error_info_(hero_common::ErrorCode::OK), max_error_(5),
...
hero_common::ErrorInfo LocalPlannerNode::Init() {
  ROS_INFO("local planner start");
  LocalAlgorithms local_algorithms;
  std::string full_path = ros::package::getPath("hero_planning") + "/local_planner/config/local_planner.prototxt";
  hero_common::ReadProtoFromTextFile(full_path.c_str(), &local_algorithms);
  if (&local_algorithms == nullptr) {
    return hero_common::ErrorInfo(hero_common::ErrorCode::LP_INITILIZATION_ERROR,
  ...
  local_cost_ = std::make_shared<hero_costmap::CostmapInterface>("local_costmap",
                                                                          *tf_,
                                                                          map_path.c_str());
  local_planner_ = hero_common::AlgorithmFactory<LocalPlannerBase>::CreateAlgorithm(selected_algorithm_);
  ...
  std::string cmd_vel_acc_pub_str = "cmd_vel_acc";
  if(local_planner_nh_.getNamespace().c_str()!="/"&&local_planner_nh_.getNamespace().size()>5)
  {
     cmd_vel_acc_pub_str = local_planner_nh_.getNamespace().substr(1) + '/' + cmd_vel_acc_pub_str;
  }

  vel_pub_ = local_planner_nh_.advertise<hero_msgs::TwistAccel>("cmd_vel_acc", 5);

  return hero_common::ErrorInfo(hero_common::ErrorCode::OK);
}

void LocalPlannerNode::ExcuteCB(const hero_msgs::LocalPlannerGoal::ConstPtr &command) {
  hero_common::ErrorInfo error_info = GetErrorInfo();
  NodeState node_state = GetNodeState();

  if (node_state == NodeState::FAILURE) {
    hero_msgs::LocalPlannerFeedback feedback;
    hero_msgs::LocalPlannerResult result;
  ...
  if (plan_mtx_.try_lock()) {
    ...
    ROS_INFO("[local_planner_->SetPlan] plan set: %s",command->route.poses.front().header.frame_id.c_str());
    plan_mtx_.unlock();
  ...
  while (ros::ok()) {
    ...
    if (node_state == NodeState::RUNNING|| node_state == NodeState::SUCCESS
      ...
      if (!error_info.IsOK()) {
        feedback.error_code = error_info.error_code();
        feedback.error_msg = error_info.error_msg();
        SetErrorInfo(hero_common::ErrorInfo::OK());
...
void LocalPlannerNode::Loop() {
  ...
  hero_common::ErrorInfo error_info = local_planner_->Initialize(local_cost_, tf_, visual_);
  if (error_info.IsOK()) {
  ...
  while (GetNodeState() == NodeState::RUNNING) {
    ...
    hero_common::ErrorInfo error_info = local_planner_->ComputeVelocityCommands(cmd_vel_);
    ...
    if (error_info.IsOK()) {
      ...
      error_info =  hero_common::ErrorInfo(hero_common::ErrorCode::LP_MAX_ERROR_FAILURE, "over max error.");
      SetNodeState(NodeState::FAILURE);
    } else {
      error_count++;
...
void LocalPlannerNode::SetErrorInfo(const hero_common::ErrorInfo error_info) {
  std::lock_guard<std::mutex> guard(node_error_info_mtx_);
  node_error_info_ = error_info;
}

void LocalPlannerNode::SetNodeState(const hero_common::NodeState& node_state) {
  std::lock_guard<std::mutex> guard(node_state_mtx_);
  node_state_ = node_state;
}

hero_common::NodeState LocalPlannerNode::GetNodeState() {
  std::lock_guard<std::mutex> guard(node_state_mtx_);
  return node_state_;
}

hero_common::ErrorInfo LocalPlannerNode::GetErrorInfo() {
  std::lock_guard<std::mutex> guard(node_error_info_mtx_);
...
void LocalPlannerNode::StartPlanning() {
  ...
  SetNodeState(hero_common::NodeState::RUNNING);
  local_planner_thread_= std::thread(std::bind(&LocalPlannerNode::Loop,this));
}

void LocalPlannerNode::StopPlanning() {
  SetNodeState(hero_common::IDLE);
  if (local_planner_thread_.joinable()) {
    local_planner_thread_.join();
  }
}

void LocalPlannerNode::AlgorithmCB(const hero_common::ErrorInfo &algorithm_error_info) {
...
int main(int argc, char **argv) {
  ...
  hero_local_planner::LocalPlannerNode local_planner;
